...
export default function GlyphPortal({
  ...
  // Prop-derived, not useId(): Next's App Router can hand useId() a different
  // per-request id between the SSG/RSC pass and client hydration, which
  // mismatches this id inside the injected <style> text and forces the whole
  // page to client-render. Stable as long as `word` is unique per page.
  ...
  for (const char of Array.from(text)) {
    const prev = characters[characters.length - 1];
    characters.push({ char, index: prev ? prev.index + prev.char.length : 0 });
  }
  ...
  return (
    <section ref={sectionRef} id={uid} className={className}
      aria-label={text} style={{ "--gp-length": length, "--gp-characters": Array.from(text).length, ...style } as CSSProperties}>
      <style>{`
